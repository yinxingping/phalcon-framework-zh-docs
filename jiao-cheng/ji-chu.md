...
接下来，我们注册一个基础URI，以便所有由Phalcon产生的URI都包含前面设置的"tutorial"文件夹。当我们后面用类 [Phalcon\Tag](https://docs.phalconphp.com/en/3.2/api/Phalcon_Tag) 生成链接时这会很重要。
...
浏览器输出应该是一样的。当Action执行结束时 Phalcon\Mvc\View 静态组件自动被创建。学习关于视图的更多用法请点[这里](https://docs.phalconphp.com/en/3.2]/views)。
...
生成的HTML代码显示了一个链接到新控制器的锚（“a"）HTML标签：
